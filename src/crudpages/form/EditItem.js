...
const EditItemOverlay = ({ onCancel, formInputs, title, initialValue }) => {
  ...
  const renderInputs = (obj, data) => {
    ...
    if (obj.type === "select") {
      const index = obj.options.findIndex((x) => x.value === data[obj.listkey]);
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <section className="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
            <h2 className="text-lg font-semibold text-gray-700 capitalize dark:text-white">
              Edit {title} Data ({initialValue.id})
            </h2>

            <form>
              <div className="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                {formInputs.map((obj) => renderInputs(obj, initialValue))}
              </div>
